...
public class TranslatationUtil {
    ...
    public static Component modifyComponent(Component orig) {
        ...
        if (!LinguaBib.disableComponentMod && player != null && orig instanceof TranslatableComponent trans && !Platform.INSTANCE.hasRemote(player)) {
            return new TranslatableComponent(ServerLangManager.INSTANCE.getTranslationFor(LanguageAPI.getPlayerLanguage(player), trans.getKey()), trans.getArgs()).setStyle(orig.getStyle());
        }
        return orig;
    }
